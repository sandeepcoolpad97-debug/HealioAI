...
const s = labOnboardingStrings.profileSubmitted;
...
type LabProfileSubmittedScreenProps = {
  navigation: {
    replace: (route: string) => void;
  };
};

export const LabProfileSubmittedScreen: React.FC<LabProfileSubmittedScreenProps> = ({
  navigation,
}) => {
          ...
          {s.whatsNextItems.map((item, index) => (
            ...
            >
              <View style={styles.whatsNextIconWrap}>
                <Ionicons
                  name={WHATS_NEXT_ICONS[index] as 'document-text-outline' | 'wallet-outline' | 'flask-outline' | 'notifications-outline'}
                  size={22}
                  color={colors.successIconGreen}
                />
              </View>
